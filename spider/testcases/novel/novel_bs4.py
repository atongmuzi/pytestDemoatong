...
def jjwxc_charper(url):
    ...
    # 查找所有的章节链接和标题内容
    # list_name = soup.select('.chapter > td > span > div > a')
    list_name = soup.select('.cytable > tbody > tr > td > span > div > a:nth-of-type(1)')
    # 遍历每一个列表中的tag对象，获取链接个目录
    for item in list_name:
        # 获取链接
        if item['href'] is None:
            r_list_link = item['rel']
        else:
            r_list_link = item['href']
        # 获取标题
        r_List_title = item.text
